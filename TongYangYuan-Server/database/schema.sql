...
CREATE TABLE IF NOT EXISTS users (
    ...
    user_type ENUM('CONSULTANT', 'PARENT', 'ADMIN') NOT NULL COMMENT '用户类型：咨询师/家长/管理员',
    ...
    vip_expire_time TIMESTAMP NULL COMMENT 'VIP到期时间',
    wx_open_id VARCHAR(100) COMMENT '微信OpenID',
    wx_union_id VARCHAR(100) COMMENT '微信UnionID',
    wx_nickname VARCHAR(100) COMMENT '微信昵称',
    wx_avatar_url VARCHAR(500) COMMENT '微信头像',
    current_child_id BIGINT COMMENT '当前操作孩子ID',
...
CREATE TABLE IF NOT EXISTS consultants (
    ...
    identity_tier ENUM('BRONZE', 'SILVER', 'GOLD', 'PLATINUM', 'YELLOW_V', 'BLUE_V', 'INTERNAL') DEFAULT 'BRONZE' COMMENT '认证等级',
    ...
    is_deposit_paid BOOLEAN DEFAULT FALSE COMMENT '是否已缴纳保证金',
    certificate_url VARCHAR(500) COMMENT '资质证书URL',
    is_certificate_verified BOOLEAN DEFAULT FALSE COMMENT '资质是否已认证',
    consultation_fee DECIMAL(10,2) DEFAULT 50.00 COMMENT '咨询费用',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
