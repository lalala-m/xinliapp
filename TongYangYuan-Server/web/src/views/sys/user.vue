<template>
  <div class="mod-sys__user">
    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" border @selection-change="state.dataListSelectionChangeHandle" @sort-change="state.dataListSortChangeHandle" style="width: 100%">
      ...
      <el-table-column prop="avatar" label="头像" header-align="center" align="center" width="80">
        <template v-slot="scope">
          <el-avatar v-if="scope.row.avatar" :src="scope.row.avatar" :size="40"></el-avatar>
          <el-avatar v-else :size="40">{{ scope.row.username?.charAt(0)?.toUpperCase() }}</el-avatar>
        </template>
      </el-table-column>
